...
import com.zth.coupon.service.IRedisService;
import com.zth.coupon.service.IUserService;
import com.zth.coupon.vo.*;
...
/**
 * h1>用户服务相关的接口实现</h1>
 * 1. 所有的操作过程, 状态都保存在 Redis 中, 并通过 Kafka 把消息传递到 MySQL 中
 * 2. 为什么使用 Kafka, 而不是直接使用 SpringBoot 中的异步处理 ?
 *
 * @author 3zZ.
 * @date 2020/10/6.
 */
...
public class UserServiceImpl implements IUserService {
    ...
    public SettlementInfo settlement(SettlementInfo info) throws CouponException {
        ...
        if (processedInfo.getEmploy() && CollectionUtils.isNotEmpty(
                processedInfo.getCouponAndTemplateInfoList()
        )) {
